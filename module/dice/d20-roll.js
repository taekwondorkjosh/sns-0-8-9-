...
export default class D20Roll extends Roll {
   ...
   * @type {string}
   */
  static EVALUATION_TEMPLATE = "systems/sns/templates/chat/roll-dialog.html";
  ...
  async toMessage(messageData={}, options={}) {
    ...
    // Add appropriate advantage mode message flavor and sns roll flags
    messageData.flavor = messageData.flavor || this.options.flavor;
    if ( this.hasAdvantage ) messageData.flavor += ` (${game.i18n.localize("SNS.Advantage")})`;
    else if ( this.hasDisadvantage ) messageData.flavor += ` (${game.i18n.localize("SNS.Disadvantage")})`;

    // Add reliable talent to the d20-term flavor text if it applied
    if ( this.options.reliableTalent ) {
      const d20 = this.dice[0];
      const isRT = d20.results.every(r => !r.active || (r.result < 10));
      const label = `(${game.i18n.localize("SNS.FlagsReliableTalent")})`;
  ...
  async configureDialog({title, defaultRollMode, defaultAction=D20Roll.ADV_MODE.NORMAL, chooseModifier=false, defaultAbility, template}={}, options={}) {
    ...
    const content = await renderTemplate(template ?? this.constructor.EVALUATION_TEMPLATE, {
      ...
      chooseModifier,
      defaultAbility,
      abilities: CONFIG.SNS.abilities
    });
  ...
  _onDialogSubmit(html, advantageMode) {
    ...
    if ( form.ability?.value ) {
      ...
      this.options.flavor += ` (${CONFIG.SNS.abilities[form.ability.value]})`;
    }
