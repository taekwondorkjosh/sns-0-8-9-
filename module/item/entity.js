...
export default class Item5e extends Item {
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/sns/templates/chat/item-card.html", templateData);
  ...
  _onCreate(data, options, userId) {
    ...
    const isCharacterClass = this.parent && (this.parent.type !== "vehicle") && (this.type === "class") && (this.type !== "spaceship");
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.SNS.armorProficienciesMap[data.data?.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const weaponProf = CONFIG.SNS.weaponProficienciesMap[data.data?.weaponType]; // Player characters check proficiency
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.SNS.sourcePacks.ITEMS}.${CONFIG.SNS.spellScrollIds[level]}`;
