...
/**
  ...
  async addEmbeddedItems(items, prompt=true) {
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize('SNS.AddEmbeddedItemPromptHint')
      });
  ...
  _computeSpellcastingProgression (actorData) {
    ...
    for ( let cls of classes ) {
      ...
      switch (prog) {
        ...
        case 'psion': progression.slot += Math.floor(levels / 2); break;
    ...
    if (!isShip && !isNPC && isSingleClass && ['psion','half', 'third'].includes(caster.spellcasting.progression) ) {
      ...
      if(caster.spellcasting.progression === 'psion') {
          progression.slot = caster.levels;
      }
    }
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("sns", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);
      weight += numCoins / CONFIG.SNS.encumbrance.currencyPerWeight;
    }
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.SNS.tokenSizes[this.data.data.traits.size || "med"];
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.sns.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = mergeObject(options, {
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.sns.roll": {type: "save", abilityId }}
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "SNS.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "SNS.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "SNS.DeathSaveFailure";
      }
    }
...
/**
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("sns", "restVariant")) {
      case 'normal': restFlavor = (longRest && newDay) ? "SNS.LongRestOvernight" : `SNS.${length}RestNormal`; break;
      case 'gritty': restFlavor = (!longRest && newDay) ? "SNS.ShortRestOvernight" : `SNS.${length}RestGritty`; break;
      case 'epic':  restFlavor = `SNS.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `SNS.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "SNS.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "SNS.LongRestResultHitDice";
    else message = `SNS.${length}RestResultShort`;
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.SNS.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(!!typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( !!typeData.swarm ) {
      type = game.i18n.format('SNS.CreatureSwarmPhrase', {
        size: game.i18n.localize(CONFIG.SNS.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
