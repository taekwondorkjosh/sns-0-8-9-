...
export default class SelectItemsPrompt extends Dialog {
  constructor(items, dialogData={}, options={}) {
    ...
    this.options.classes = ["sns", "dialog", "select-items-prompt", "sheet"];
  ...
  static async create(items, {
    ...
    const html = await renderTemplate("systems/sns/templates/apps/select-items-prompt.html", {items, hint});

    return new Promise((resolve) => {
      const dlg = new this(items, {
        title: game.i18n.localize('SNS.SelectItemsPromptTitle'),
        content: html,
        buttons: {
          apply: {
            icon: `<i class="fas fa-user-plus"></i>`,
            label: game.i18n.localize('SNS.Apply'),
            callback: html => {
          ...
          cancel: {
            ...
            label: game.i18n.localize('SNS.Skip'),
            callback: () => resolve([])
          }
        },
        default: "apply",
