...
export const migrateWorld = async function() {
  ui.notifications.info(`Applying SNS System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`, {permanent: true});
  ...
  ui.notifications.info(`SnS System Migration to version ${game.system.data.version} completed!`, {permanent: true});
};
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    const typeMatch = Object.entries(CONFIG.SNS.creatureTypes).find(([k, v]) => {
      return (typeLc === k) ||
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("SNS.CreatureSwarm"));
    if ( match.groups.size || isNamedSwarm ) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.SNS.actorSizes).find(([k, v]) => {
